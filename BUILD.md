...
This guide covers how to set up the development environment and build Handier from source across different platforms.
...
```bash
git clone git@github.com:MagicNoThief/Handier-desktop.git
cd Handier-desktop
```
...
The raw binary (`src-tauri/target/release/handier`) cannot run standalone — it needs Tauri resource files (tray icons, sounds, VAD model) to be co-located at the expected path.

**Install from the deb bundle** (works on any Linux distro):

```bash
cd /tmp
ar x /path/to/Handier-desktop/src-tauri/target/release/bundle/deb/Handier_*_amd64.deb data.tar.gz
tar xzf data.tar.gz
sudo cp usr/bin/handier usr/bin/handier-llm /usr/bin/
sudo cp -a usr/lib/. /usr/lib/
sudo cp -r usr/share/icons/hicolor/* /usr/share/icons/hicolor/
sudo cp usr/share/applications/Handier.desktop /usr/share/applications/
```

The runtime libraries live in the app-private `/usr/lib/Handier/` (on the binary's rpath), so no `ldconfig` step is needed.

After subsequent rebuilds, copy the binary and any refreshed runtime libraries:

```bash
sudo cp src-tauri/target/release/handier /usr/bin/
sudo mkdir -p /usr/lib/Handier
sudo cp -a src-tauri/transcribe-libs/. /usr/lib/Handier/
```
...
remains visibly enabled, leaving Handier on `Waiting...`.

After installing the final bundle at `/Applications/Handier.app`, quit Handier, clear only its
...
```bash
codesign -dr - /path/to/previous/Handier.app 2>&1
codesign -dr - /Applications/Handier.app 2>&1
```
...
~/.cache/tauri/linuxdeploy-x86_64.AppImage --appimage-extract-and-run \
  --appdir Handier.AppDir --plugin gtk --output appimage
```
...
# Rust projects' build output, not just Handier):
...
If the build compiles all the way to `Built application at: ...\handier.exe` and
then fails with:

```
Signing C:\...\handier.exe with a custom signing command
